...
https:\/\/ios\.baertt\.com\/v5\/Nameless\/adlick(start|end)\.json url script-request-body youth_kkz.js
...
if ($.isNode()) {
  if (process.env.YOUTH_KKZ_START && process.env.YOUTH_KKZ_START.indexOf('&') > -1) {
  StartBody = process.env.YOUTH_KKZ_START.split('&');
  }else {
  StartBody = process.env.YOUTH_KKZ_START.split()
  };
   if (process.env.YOUTH_KKZ_END && process.env.YOUTH_KKZ_END.indexOf('&') > -1) {
  EndBody = process.env.YOUTH_KKZ_END.split('&');
  } else {
  EndBody = process.env.YOUTH_KKZ_END.split()
  }
} else {
  StartBody = $.getdata('youth_kkz_start').split('&');
  EndBody = $.getdata('youth_kkz_end').split('&');
}
